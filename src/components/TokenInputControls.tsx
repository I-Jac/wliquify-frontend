...
export const TokenInputControls: React.FC<TokenInputControlsProps> = ({
    ...
    const inputPlaceholder = t('tokenTable.inputControls.amountPlaceholder', { symbol: isDeposit ? symbol : 'wLQI' });
        ...
        <div className="flex flex-col space-y-1">
            <div className="flex items-center justify-between">
                <div className="text-gray-400 text-[10px] flex items-center">
                    ...
                    <span>{t('global.balance', { value: displayUserBalance, symbol: isDeposit ? symbol : 'wLQI' })}</span>
                </div>
                <div className="flex space-x-1">
                    <button
                        ...
                        title={t('tokenTable.inputControls.half')}
                    >
                        {t('tokenTable.inputControls.half')}
                    </button>
                    <button
                        onClick={() => handleSetAmount(mintAddress, action, 1)}
                        disabled={actionDisabled || userBalance === null || (isDeposit && isDelisted)}
                        className={`${buttonClassName} ${(actionDisabled || userBalance === null || (isDeposit && isDelisted)) ? 'cursor-not-allowed opacity-50' : ''}`}
                        title={t('tokenTable.inputControls.max')}
                    >
                        {t('tokenTable.inputControls.max')}
                    </button>
                </div>
            </div>
            <div className="relative w-full">
                ...
                {showTargetButton && handleSetTargetAmount && (
                    <button
                        ...
                        title={t('tokenTable.inputControls.toTarget')}
                    >
                        {t('tokenTable.inputControls.toTarget')}
                    </button>
                )}
            </div>
